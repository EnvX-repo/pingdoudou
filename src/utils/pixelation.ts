...
export function findClosestPaletteColor(
  ...
  // 计算目标颜色的亮度（用于判断是否接近黑/白）
  const targetLuma = targetRgb.r * 0.299 + targetRgb.g * 0.587 + targetRgb.b * 0.114;
  const targetSum = targetRgb.r + targetRgb.g + targetRgb.b;
  
  // 在调色板中找黑色和白色
  const blackColor = palette.find(p => p.hex.toUpperCase() === '#000000');
  const whiteColor = palette.find(p => p.hex.toUpperCase() === '#FFFFFF');
  
  // 如果目标颜色接近黑色（亮度低或RGB总和小），优先映射到黑色
