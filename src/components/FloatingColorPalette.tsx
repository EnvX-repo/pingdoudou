...
interface FloatingColorPaletteProps {
  ...
  remainingColors: { key: string; color: string }[];
...
const FloatingColorPalette: React.FC<FloatingColorPaletteProps> = ({
  ...
  // 渲染颜色网格的辅助函数
  const renderColorGrid = (colorList: { key: string; color: string }[], title: string) => (
    <div className="mb-4">
      <div className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2 px-1">
        {title} ({colorList.length})
      </div>
        ...
        {colorList.map((colorData) => {
          ...
          return (
            <button
              ...
              style={{ backgroundColor: colorData.color, pointerEvents: 'auto' }}
    ...
    >
      ...
      <div className="p-3 max-h-[520px] overflow-y-auto">
          ...
          <div className="flex gap-2 mb-3 flex-wrap">
            ...
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                // 选择橡皮擦时，调用onColorSelect
                onColorSelect({ key: TRANSPARENT_KEY, color: '#FFFFFF', isExternal: true });
              }}
              className={`flex-1 p-2 rounded-lg border transition-all duration-200 flex items-center justify-center gap-1 text-xs min-w-[70px] ${
                selectedColor?.key === TRANSPARENT_KEY || selectedColor?.key === 'ERASE'
            ...
            <button
              ...
              style={{ pointerEvents: 'auto' }}
              title="区域填充：先选颜色，再点击画布上要填充的区域，该封闭区域会被填成当前颜色"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="space-y-4">
            {/* 第一栏：当前色板 */}
            {renderColorGrid(colors, '当前色板')}
            
            {/* 第二栏：余色色板（用户选择的颜色中，图纸未使用的） */}
            {renderColorGrid(remainingColors, '余色色板')}
            
            {/* 第三栏：全部色板 */}
            {renderColorGrid(fullPaletteColors, '全部色板')}
          </div>
