...
import { TRANSPARENT_KEY, transparentColorData, floodFillPaint, recalculateColorStats } from '../utils/pixelEditingUtils';
...
function OriginalHomePage() {
  ...
  useEffect(() => {
    ...
    if (imageUrl) {
      ...
      setOriginalImageSrc(decodedImageUrl);
    }

    if (colorsParam) {
      // 解析颜色限制，只允许使用这些颜色
      const allowedColors = colorsParam.split(',').map(c => c.toUpperCase());
      
      // 更新自定义色板选择，只选择允许的颜色
      const restrictedSelections: PaletteSelections = {};
      allowedColors.forEach(hex => {
        if (/^#[0-9A-F]{6}$/i.test(hex)) {
          restrictedSelections[hex.toUpperCase()] = true;
        }
      });

      if (Object.keys(restrictedSelections).length > 0) {
        setCustomPaletteSelections(restrictedSelections);
        setIsCustomPalette(true);
      }
    }
  }, [searchParams]);
  ...
  const handleCanvasInteraction = (
    ...
    if (i >= 0 && i < N && j >= 0 && j < M) {
      ...
      // 区域填充模式：用当前选中颜色填充与点击处同色且连通的区域
      if (isClick && isFillMode && selectedColor && selectedColor.key !== TRANSPARENT_KEY && selectedColor.key !== 'ERASE') {
        if (mappedPixelData && gridDimensions && cellData && !cellData.isExternal) {
          const newPixelData = floodFillPaint(
            mappedPixelData,
            gridDimensions,
            j,
            i,
      ...
      <header className="w-full md:max-w-4xl text-center mt-6 mb-8 sm:mt-8 sm:mb-10 relative overflow-hidden">
        ...
        <div className="relative z-10 py-8">
          ...
          <div className="mt-6 flex flex-col items-center justify-center space-y-2">
            ...
            <a
              href="/consume-beads"
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md border border-green-200 dark:border-green-700 bg-green-50 dark:bg-green-900/20 text-xs text-gray-700 dark:text-gray-200 font-semibold hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 text-green-500 dark:text-green-300">
